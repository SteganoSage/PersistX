...
void Shell::cmd_buffer()
{
    ...
    for (auto &f : frames)
    {
        ...
        rows.push_back({std::to_string(f.frame_id),
    ...
    std::cout << "  " << term::dim(std::to_string(rows.size()) + " used, " + std::to_string(empty) + " free of " + std::to_string(frames.size()) + " frames") << "\n\n";
}
...
void Shell::cmd_wal(const std::vector<std::string> &args)
{
    ...
    std::cout << "\n";
    term::print_table(headers, rows);
    std::cout << "  " << term::dim("Showing " + std::to_string(rows.size()) + " of " + std::to_string(records.size()) + " records") << "\n\n";
}
...
void Shell::cmd_page_cmd(const std::vector<std::string> &args)
{
    ...
    try
    {
        pid = static_cast<page_id_t>(std::stoul(args[1]));
    }
    catch (...)
    {
    ...
    if (node_type == 0 || node_type == 1)
    {
        ...
        term::print_box("Page " + std::to_string(pid) + " (" + type_str + ")",
                        {{"Node type:", type_str},
                         ...
                         {"Parent:", parent == INVALID_PAGE_ID ? "none" : std::to_string(parent)}},
                        34);
    }
    else
    {
        ...
        term::print_box("Page " + std::to_string(pid) + " (DATA)",
                        {{"Page ID:", std::to_string(page->get_page_id())},
                         ...
                         {"Page LSN:", std::to_string(page->get_page_lsn())}},
                        34);
    }
...
void Shell::cmd_recover()
{
    ...
    for (uint32_t pid = 0; pid < num_pages; ++pid)
    {
        ...
        for (uint16_t s = 0; s < slot_count; ++s)
        {
            ...
            if (!page->read_record(s, data))
                continue; // tombstone or empty
...
void Shell::cmd_fill(const std::vector<std::string> &args)
{
    ...
    bool owned_txn = (active_txn_ == nullptr);
    Transaction *txn = owned_txn ? txn_mgr_->begin() : active_txn_;

    for (int i = 0; i < n; ++i)
    {
        int64_t key = next_fill_key_++;
        ...
        // Pass index_key so abort() can remove the B+ tree entry if needed.
